// ข้อมูลคลินิกตั้งต้น (สำหรับแสดงผลเมื่อไม่มีการเพิ่มข้อมูลใหม่จากแอดมิน)
...
// ดึงไฟล์เปรียบเทียบคลินิกร่วมกันระหว่างหน้าผู้ใช้งานหลักและหน้าแอดมิน
let clinicsData = JSON.parse(localStorage.getItem('unni_clinics')) || defaultClinics;
let reviews = JSON.parse(localStorage.getItem('unni_premium_reviews')) || [];
...
// หากยังไม่มีการจัดเก็บบันทึกคลินิกใด ๆ ให้เซฟค่าตั้งต้นลงไปเพื่อพร้อมทำงานร่วมกับหน้า Admin
...
function goToTab(tabId) {
    ...
    // โหลดข้อมูลเพื่ออัปเดตหน้าแต่ละหน้าให้สดใหม่เสมอเมื่อทำการคลิก
    if (tabId === 'clinics') {
        // ดึงรายชื่อคลินิกใหม่แบบเรียลไทม์ (เผื่อแอดมินมีการเพิ่มข้อมูล)
        clinicsData = JSON.parse(localStorage.getItem('unni_clinics')) || defaultClinics;
        renderClinics();
    }
    if (tabId === 'profile') {
        renderSavedClinics();
...
// ผูกการทำงานทั้งหมดเมื่อโหลดหน้าเว็บเสร็จ
document.addEventListener('DOMContentLoaded', () => {
    // ดึงข้อมูลรีวิวล่าสุดเสมอ
    reviews = JSON.parse(localStorage.getItem('unni_premium_reviews')) || [];
    renderReviews();
    renderClinics();
...
// เรนเดอร์การ์ดรีวิวพร้อมสไลเดอร์รูดภาพ
function renderReviews() {
    const reviewsContainer = document.getElementById('reviewsContainer');
    if (!reviewsContainer) return;
    reviewsContainer.innerHTML = '';
    
    // โหลดฐานข้อมูลใหม่ล่าสุด (รองรับกรณีแอดมินลบรีวิว)
    reviews = JSON.parse(localStorage.getItem('unni_premium_reviews')) || initialReviews;
...
function renderBookingRecords() {
    ...
    // โหลดประวัติการจองจากฐานข้อมูลแบบล่าสุด (รองรับกรณีแอดมินลบประวัติการจอง)
